...
def _get_depth_two_zz_pqc_layer(gan_instance):
    """Validate the two independent layer angles used by a depth-2 ZZ landscape."""
    if gan_instance.n_layers != 2:
        raise ValueError(
            "The ZZ loss landscape is two-dimensional only for n_layers=2. "
            f"Received n_layers={gan_instance.n_layers}."
        )

    pqc_layer = gan_instance.generator.get_layer('re-uploading_PQC')
    weights = pqc_layer.get_weights()
    if len(weights) != 1 or weights[0].shape != (2,):
        raise ValueError(
            "Expected one ZZ angle per layer with weight shape (2,), "
            f"but received {[weight.shape for weight in weights]}."
        )
    return pqc_layer
...
def plot_true_wasserstein_landscape(gan_instance, batch_size=1000, resolution=20, save_folder='./'):
    ...
    The two coordinates are the shared interaction angles of layers 1 and 2.
    Reuses only depth-2 ZZ points from its dedicated CSV cache.
    ...
    filename = "zz_depth2_true_emd_landscape.csv"
    html_filename = "interactive_zz_depth2_true_emd_landscape.html"
    ...
    pqc_layer = _get_depth_two_zz_pqc_layer(gan_instance)
    ...
    fig.update_layout(
        ...
        scene=dict(
            xaxis=dict(title='Layer 1 interaction angle', tickvals=pi_ticks, ticktext=pi_ticktexts),
            yaxis=dict(title='Layer 2 interaction angle', tickvals=pi_ticks, ticktext=pi_ticktexts),
    ...
    fig.write_html(os.path.join(save_folder, html_filename))
    fig.show()
...
def plot_garch_landscape(gan_instance, batch_size=1000, resolution=50, folder='./'):
    ...
    GARCH(1,1) parameters (alpha and beta) directly sampling over:
      - Layer 1 interaction angle in [-pi/2, 3pi/2]
      - Layer 2 interaction angle in [-pi, pi]
    ...
    filename = "zz_depth2_garch_landscape.csv"
    html_filename = "interactive_zz_depth2_garch_landscape.html"
    ...
    fig.update_layout(
        ...
        scene=dict(
            xaxis=dict(title='Layer 1 interaction angle', tickvals=t1_ticks, ticktext=t1_ticktexts),
            yaxis=dict(title='Layer 2 interaction angle', tickvals=t2_ticks, ticktext=t2_ticktexts),
