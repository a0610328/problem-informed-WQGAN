...
class quantum_GAN(object):
    def __init__(self, chopsize, stride, n_qubits, n_layers, epochs, save=False, run_id=1, path='./', resume_path=None, from_epoch = 0):    
        ...
        self.metrics_real= st.calc_unbiased_window_metrics(self.data_real_2d, self.max_lag)
        ...
        try:
            ...
            for epoch in range(self.from_epoch, self.epochs):    
                ...
                if epoch % self.eval_interval == 0:
                    ...
                    mean_abs_gen, _, mean_nonabs_gen, _, mean_lev_gen, _ = st.calc_unbiased_window_metrics(generated_data_transformed, max_lag=self.max_lag)
                    mean_abs_real, _, mean_nonabs_real, _, mean_lev_real, _ = self.metrics_real
                ...
                if epoch % 100 == 0:
                    fig = st.QQ_plot(generated_data_transformed, self.log_returns, 'QQ plot at epoch {}'.format(str(epoch)), xlabel = 'Quantiles generated', ylabel = 'Quantiles BTC', limit = [-0.04,0.04], show = False, path = self.plot_path+'/QQ_plot_epoch_{}.pdf'.format(str(epoch )))
                    fig.savefig(self.plot_path+'/QQ_plot_epoch_{}.pdf'.format(str(epoch )))
                    plt.close(fig)  # Crucial: frees the bitmap/canvas memory
    ...
    def generate_samples(self, weights_path):
        ...
        # Directly inverse transform (scale_factor is handled natively by the Alternating layer)
        generated_data_transformed = dh.inverse_transform(generated_data, self.transform_params)

        st.QQ_plot(generated_data_transformed, self.log_returns, 'QQ plot at best epoch', xlabel = 'Quantiles generated', ylabel = 'Quantiles BTC', limit = [-0.04,0.04], show = True, path = './QQ_best.pdf')
    ...
    def load_models_custom(self, resume_path, epoch):
        ...
        try:
            with open(f"{resume_path}/generator_epoch_{epoch}.pkl", "rb") as f:
                gen_weights = pickle.load(f)
                self.generator.set_weights(gen_weights)
                
            with open(f"{resume_path}/discriminator_epoch_{epoch}.pkl", "rb") as f:
