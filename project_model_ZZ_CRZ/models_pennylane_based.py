...
def generate_circuit(qubits, n_layers, params, inputs):
    ...
    for l in range(n_layers):
        ...
        # A. Encoding Layer
        for i in range(len(qubits)):
            qml.RX(inputs[:, l, 2*i], wires=qubits[i])
            qml.RZ(inputs[:, l, 2*i+1], wires=qubits[i])

        # B. Entangling Layer from interaction
        for i in range(n_qubits - 1):
            qml.PauliRot(params[0, l], 'XX', wires = [qubits[i], qubits[i + 1]])

        qml.PauliRot(params[0, l], 'XZ', wires = [qubits[n_qubits - 1], qubits[0]])

        for i in range(n_qubits - 1):
            qml.PauliRot(params[0, l], 'ZZ', wires = [qubits[i], qubits[i + 1]])

        # Leverage control layer
        for i in range(n_qubits - 1):

            qml.Hadamard(wires = qubits[i])
            qml.PauliX(wires = qubits[i])
            qml.CRZ(params[1, l], wires = [qubits[i], qubits[i + 1]])
            qml.PauliX(wires = qubits[i])
            qml.Hadamard(wires = qubits[i])
            

        qml.Hadamard(wires = qubits[n_qubits - 1])
        qml.PauliX(wires = qubits[n_qubits - 1])
        qml.CRX(params[1, l], wires = [qubits[n_qubits - 1], qubits[0]])
        qml.PauliX(wires = qubits[n_qubits - 1])
        qml.Hadamard(wires = qubits[n_qubits - 1])
        ...
        for i in range(n_qubits - 1):
            qml.PauliX(wires = qubits[i])
            qml.CRX(params[1, l], wires = [qubits[i], qubits[i + 1]])
            qml.PauliX(wires = qubits[i])
